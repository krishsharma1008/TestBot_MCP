...
const CODEBASE_CONTEXT_SCHEMA = z
  .object({
    pages: z.array(z.any()).optional(),
    apiEndpoints: z.array(z.any()).optional(),
    workflows: z
      .array(z.union([z.string().min(1).max(500), WORKFLOW_OBJECT_SCHEMA]))
      .optional(),
  })
  .passthrough()

const PLAYWRIGHT_MCP_OPTIONS_SCHEMA = z
  .object({
    enabled: z.boolean().optional(),
    mcpPackageName: z.literal("@playwright/mcp").optional(),
    mcpVersion: z
      .string()
      .regex(/^(?!latest$)[0-9A-Za-z._-]+$/)
      .optional(),
    noInstall: z.boolean().optional(),
  })
  .optional()

const RESULT_MERGE_OPTIONS_SCHEMA = z
  .object({
    dedupeStrategy: z.enum(["legacy", "strict"]).optional(),
  })
  .optional()

const LOG_REDACTION_OPTIONS_SCHEMA = z
  .object({
    enabled: z.boolean().optional(),
...
class HealixMCPServer {
  constructor() {
    Logger.initialize()
    console.error(
      "[DEBUG] Healix MCP Server starting - VERSION WITH ZOD SCHEMAS",
    )
  ...
  createConfigUILauncher(config = {}) {
    ...
    if (this._activeConfigUILauncher) {
      try {
        this._activeConfigUILauncher.cancel()
      } catch (_) {}
  ...
  findRecentRunForProject(projectPath) {
    try {
      ...
      Logger.warn("Index", "findRecentRunForProject failed", {
        reason: err.message,
      })
      return null
    }
  ...
  emitRunStatusTelemetry(statusPayload) {
    ...
    const status =
      phase === "completed"
        ? "success"
        : phase === "error" || phase === "error_reported"
          ? "error"
          : "info"
    ...
    this.emitTelemetry({
      ...
      metadata: {
        ...
        generationProvider:
          statusPayload.generationMeta?.selectedGenerator ||
          statusPayload.generationMeta?.provider ||
          null,
      },
    })
  }
  ...
  detectProjectFramework(projectPath) {
    try {
      ...
      if (deps.expo || deps["expo-router"])
        return { kind: "expo", defaultPort: 8081 }
  ...
  autoCorrectPortConfig({ projectPath, startCommand, port, baseURL }) {
    ...
    if (
      ...
      finalPort === framework.defaultPort &&
  ...
  persistUploadedPrdFiles(statusDir, prdFiles) {
    ...
    prdFiles.forEach((prdPayload, index) => {
      ...
      const originalName = path.basename(
        prdPayload.name || `uploaded-prd-${index}.md`,
      )
      const ext = path.extname(originalName).toLowerCase()
      const safeExt = allowedExtensions.has(ext) ? ext : ".md"
      const baseName = path
  ...
  validateUISubmission(rawConfig) {
    ...
    if (!parsed.success) {
      ...
      const error = new Error(
        `Invalid configuration payload: ${firstIssue?.message || "unknown error"}`,
      )
  ...
  resolveAutoOpenBrowserPreference(
    params = {},
    ...
    const envAutoOpen = resolveBoolean(
      process.env.HEALIX_AUTO_OPEN_BROWSER,
      false,
    )
  ...
  createBasePipelineConfig(context, params) {
    const normalizedCodebase = this.normalizeCodebaseContext(
      params.codebaseContext,
    )
    const hasLocalProject = this.looksLikeLocalProjectPath(context.projectPath)
    const pageCount = Array.isArray(normalizedCodebase?.pages)
      ? normalizedCodebase.pages.length
      : 0
    const workflowCount = Array.isArray(normalizedCodebase?.workflows)
      ? normalizedCodebase.workflows.length
      : 0
    const shouldAutoUpgrade =
      hasLocalProject && (pageCount > 0 || workflowCount > 0)
    ...
    if (shouldAutoUpgrade && params.strictAIGeneration === false) {
      ...
      configAutoUpgrades.push({
        ...
        detail:
          "Promoted strictAIGeneration=false → true for local project with detected pages/workflows",
    ...
    if (
      shouldAutoUpgrade &&
      (params.generationMode === "template-only" ||
        params.generationMode === "saas-only")
    ) {
    ...
    let minGeneratedTests =
      Number.isFinite(parsedMinGeneratedTests) && parsedMinGeneratedTests > 0
    ...
    let coverageProfile = params.coverageProfile || "balanced"
    if (shouldAutoUpgrade && params.coverageProfile === "balanced") {
      coverageProfile = "qa-max"
      configAutoUpgrades.push({
        kind: "config_auto_upgraded",
        detail:
          "Promoted coverageProfile=balanced → qa-max for local project with detected pages/workflows",
    ...
    if (configAutoUpgrades.length > 0) {
      Logger.info(
        "Index",
        `Auto-upgraded ${configAutoUpgrades.length} generation config field(s) for local project`,
        {
          projectPath: context.projectPath,
          pageCount,
          workflowCount,
          upgrades: configAutoUpgrades.map(
            (u) => `${u.field}:${u.from}→${u.to}`,
          ),
        },
      )
    }
    ...
    const autoOpenBrowser = this.resolveAutoOpenBrowserPreference(
      params,
      headless,
    )
      ...
      services:
        Array.isArray(params.services) && params.services.length > 0
          ...
          : Array.isArray(context.services)
            ...
            : undefined,
      apiOnly:
        typeof params.apiOnly === "boolean" ? params.apiOnly : detectedApiOnly,
      jira: params.jira,
  ...
  async continuePipelineAfterConfig({
    ...
    try {
      ...
      const prdFiles = this.persistUploadedPrdFiles(
        statusDir,
        validatedConfig.prdFiles,
      )
      ...
      if (corrected.appliedFixes && corrected.appliedFixes.length > 0) {
        for (const fix of corrected.appliedFixes) {
          Logger.info(
            "Index",
            `Config auto-correction [${fix.kind}]: ${fix.detail}`,
          )
        }
      }
      ...
      const allAutoFixes = [
        ...(Array.isArray(baseConfig.configAutoFixes)
          ? baseConfig.configAutoFixes
          : []),
      ...
      Logger.error("Index", "Configuration UI flow failed", error, {
        runId,
        errorCode,
      })
    }
  ...
  runPipelineInBackground(config, runId, statusDir) {
    ...
    Logger.info("Index", `Forking pipeline worker in background`, {
      runId,
    ...
    const _killWorkerPid = (pidFile) => {
      ...
      if (pid > 0) {
        try {
          ...
          Logger.info("Index", "Killed leftover Healix pipeline worker", {
            pid,
          })
        } catch {
          /* already gone */
    ...
    try {
      fs.mkdirSync(healixReportsDir, { recursive: true })
    } catch {
      /* ignore */
    }
    ...
    const resolvedStatusDir =
      statusDir ||
      path.join(config.projectPath, "healix-reports", ".runs", runId)
    ...
    try {
      ...
      Logger.warn(
        "Index",
        "Could not write config temp file; falling back to IPC send",
    ...
    try {
      ...
      process.stderr.write(
        `[HEALIX] Failed to send config to worker: ${sendErr.message}\n`,
      )
    }
    ...
    const WORKER_TERMINAL_PHASES = new Set([
      "completed",
      "error",
      "error_reported",
      "failed",
    ...
    child.on("exit", (code, signal) => {
      ...
      if (code === null && (signal === "SIGKILL" || signal === "SIGTERM"))
        return
      try {
        let existingPhase = null
        if (fs.existsSync(crashStatusFile)) {
          try {
            existingPhase = JSON.parse(
              fs.readFileSync(crashStatusFile, "utf-8"),
            ).phase
          } catch {
            /* ignore parse errors */
          }
        ...
        if (!existingPhase || !WORKER_TERMINAL_PHASES.has(existingPhase)) {
          ...
          const errorCode = isCleanButUnfinished
            ? "WORKER_SILENT_EXIT"
            : "WORKER_CRASH"
        ...
        process.stderr.write(
          `[HEALIX] Could not write crash status: ${e.message}\n`,
        )
      }
    })
    ...
    if (sendError) {
      ...
      try {
        ...
        process.stderr.write(
          `[HEALIX] Could not write IPC-send error status: ${e.message}\n`,
        )
      }
    }
    ...
    if (child.pid) {
      try {
        fs.writeFileSync(workerPidFile, String(child.pid))
      } catch {
        /* non-fatal */
      }
    }
  ...
  async waitForPipelineCompletion(statusFile, maxWaitMs = 1800000) {
    ...
    return {
      phase: "timeout",
      message: "Healix test run monitoring timed out after 30 minutes.",
    }
  }
  ...
  registerTools() {
    this.server.registerTool(
      ...
      {
        description:
          "Analyze a project and return configuration options before testing. Use this first to understand the project structure, then use the returned configuration with healix_test_my_app. Returns detected settings and questions for the user to answer.",
        inputSchema: z.object({
          projectPath: z
            .string()
            .optional()
            .describe(
              "Path to the project to analyze (defaults to current workspace)",
            ),
        }),
      },
      async (args, extra) => {
        const telemetryStartedAt = this.trackToolInvocation(
          "healix_configure",
          args,
        )
        console.error(
          "[DEBUG] healix_configure called, projectPath:",
          args?.projectPath,
        )
        Logger.mcp("Index", `Tool called: healix_configure`, {
    ...
    this.server.registerTool(
      "healix_test_my_app",
      {
        ...
        inputSchema: z.object({
          ...
          generateTests: z
            ...
            .optional()
            .describe(
              "Whether to generate new tests (true) or use existing tests (false)",
            ),
          prdFile: z
            .string()
            .optional()
            .describe(
              "Path to PRD/requirements document for test generation (optional)",
            ),
          codebaseContext: CODEBASE_CONTEXT_SCHEMA.optional().describe(
            "Structured codebase context from AI agent analysis (pages, apiEndpoints, workflows)",
          ),
          baseURL: z
            .string()
            .optional()
            .describe("Base URL for the application under test"),
          port: z.number().optional().describe("Port number the app runs on"),
          startCommand: z
            .string()
            .optional()
            .describe('Command to start the app server (e.g., "npm start")'),
          jira: z
            .object({
              enabled: z.boolean().optional(),
            ...
            .optional()
            .describe("Jira integration configuration"),
          openDashboard: z
            .boolean()
            .optional()
            .describe(
              "Whether to prepare/open dashboard output after tests (default: true)",
            ),
          headless: z
            .boolean()
            .optional()
            .describe(
              "Run in headless mode (default: true). Prevents auto-opening browser windows from MCP.",
            ),
          autoOpenBrowser: z
            .boolean()
            .optional()
            .describe(
              "Allow browser auto-open for config/dashboard pages (default: false, ignored when headless=true).",
            ),
          generationMode: z
            .enum(["openai-first", "openai-only", "template-only", "saas-only"])
            .optional()
            .describe("Generation strategy"),
          strictAIGeneration: z
            .boolean()
            .optional()
            .describe(
              "Enforce AI-only generation with no template fallback (default: true)",
            ),
          minGeneratedTests: z
            .number()
            .int()
            ...
            .optional()
            .describe(
              "Minimum generated tests required before execution (default: 50)",
            ),
          coverageProfile: z
            .enum(["balanced", "qa-max", "exhaustive"])
            .optional()
            .describe(
              "Generation depth and coverage profile (default: qa-max)",
            ),
          phaseMode: z
            .enum(["single", "two-phase"])
            .optional()
            .describe(
              "Execution mode: single pass or gate+deep two-phase (default: two-phase)",
            ),
          serverStartTimeoutMs: z
            .number()
            .int()
            .min(10000)
            .max(300000)
            .optional()
            .describe(
              "Server startup timeout in ms before failing readiness checks (default: 90000)",
            ),
          serverHealthCheckIntervalMs: z
            .number()
            .int()
            .min(250)
            .max(5000)
            .optional()
            .describe(
              "Interval in ms between server readiness probes (default: 1000)",
            ),
          artifactMode: z
            .enum(["hybrid", "full"])
            .optional()
            .describe("Artifact capture mode"),
          browserMode: z
            .enum(["chromium", "smoke-matrix", "full-matrix"])
            .optional()
            .describe("Browser execution mode"),
          validateGeneratedTests: z
            .boolean()
            .optional()
            .describe("Validate generated tests before execution"),
          aiFailureAnalysis: z
            .boolean()
            .optional()
            .describe("Enable AI analysis for failed tests"),
          showMouseCursorInVideo: z
            .boolean()
            .optional()
            .describe(
              "Render synthetic mouse cursor overlay in generated Playwright video output (default: true)",
            ),
          playwrightMcp: PLAYWRIGHT_MCP_OPTIONS_SCHEMA.describe(
            "Playwright MCP execution options",
          ),
          resultMerge: RESULT_MERGE_OPTIONS_SCHEMA.describe(
            "Result merge options",
          ),
          logRedaction: LOG_REDACTION_OPTIONS_SCHEMA.describe(
      ...
      async (args, extra) => {
        ...
        console.error(
          "[DEBUG] healix_test_my_app called, projectPath:",
          args?.projectPath,
        )
        Logger.mcp("Index", `Tool called: healix_test_my_app`, {
          projectPath: args?.projectPath,
    ...
    this.server.registerTool(
      ...
      async (args, extra) => {
        ...
        Logger.mcp("Index", `Tool called: healix_check_run_status`, {
          runId: args?.runId,
        })
        try {
          await this.validateApiKey()
    ...
    this.server.registerTool(
      ...
      {
        description:
          "Analyze existing test failures with AI without running new tests",
        inputSchema: z.object({
          projectPath: z.string().describe("Path to the project"),
          testResultsPath: z
            .string()
            .optional()
            .describe("Path to test-results.json file"),
          // aiProvider is deprecated — all AI calls now proxy through the Healix webapp using HEALIX_API_KEY.
          aiProvider: z
            .enum(["saas"])
            .optional()
            .describe(
              'Deprecated. Only "saas" is supported; all AI proxies through the Healix webapp.',
            ),
        }),
      },
      async (args, extra) => {
        ...
        Logger.mcp("Index", `Tool called: healix_analyze_failures`, {
          projectPath: args?.projectPath,
        })
        try {
          await this.validateApiKey()
      ...
      "healix_generate_report",
      {
        ...
        inputSchema: z.object({
          ...
          openDashboard: z
            .boolean()
            .optional()
            .describe("Whether to automatically open the dashboard"),
        }),
      },
      async (args, extra) => {
        const telemetryStartedAt = this.trackToolInvocation(
        ...
        Logger.mcp("Index", `Tool called: healix_generate_report`, {
          projectPath: args?.projectPath,
        })
        try {
          await this.validateApiKey()
  ...
  async validateApiKey() {
    ...
    if (!dashboardUrl) {
      ...
      const webappAlive = await defaultProbeWebappHealth(
    ...
    for (
      let attempt = 0;
      attempt < VALIDATE_RETRY_DELAYS_MS.length;
      attempt++
    ) {
      if (VALIDATE_RETRY_DELAYS_MS[attempt] > 0) {
        await new Promise((r) =>
          setTimeout(r, VALIDATE_RETRY_DELAYS_MS[attempt]),
        )
        Logger.warn(
          "Index",
          `Retrying API key validation (attempt ${attempt + 1}/${VALIDATE_RETRY_DELAYS_MS.length})`,
          {
      ...
      try {
        response = await fetch(
          `${effectiveDashboardUrl}/api/mcp-auth/validate`,
          {
            method: "POST",
    ...
    if (!response && lastNetworkErr) {
      Logger.error(
        "Index",
        "API key validation request failed (network/timeout)",
  ...
  setupErrorHandling() {
    ...
    process.on("unhandledRejection", (reason, promise) => {
      Logger.error("Index", `[Healix MCP Unhandled Rejection]`, {
        reason,
        promise,
      })
    })
  ...
  async handleConfigure(params) {
    Logger.mcp("Index", "handleConfigure called", {
    ...
    try {
      ...
      Logger.info(
        "Index",
        `Detected project: ${context.projectName} (${context.language})`,
      )
      Logger.info(
        "Index",
        `Framework detection: ${context.hasPlaywright ? "Playwright found" : "No Playwright config"}`,
      )
        ...
        questions: [
          {
            ...
            default: "both",
            description:
              "Choose frontend for UI tests, backend for API tests, or both for full coverage",
          },
          {
            id: "generateTests",
            prompt:
            ...
            description:
              "generate_new creates tests from codebase analysis, use_existing runs your current tests",
          },
        ],
      ...
      if (prdFiles.length > 0) {
        config.questions.push({
          ...
          default: "yes",
          description:
            "PRD files help generate more accurate tests based on requirements",
        })
      }
      ...
      if (hasJiraConfig) {
        config.questions.push({
          id: "useJira",
          prompt:
            "Jira integration is configured. Fetch stories for test generation?",
          options: ["yes", "no"],
          default: "no",
          description:
            "Fetch active Jira stories and generate tests from acceptance criteria",
        })
      }
...
`
  ...
  async handleTestMyApp(params) {
    ...
    Logger.mcp("Index", "handleTestMyApp called", {
      projectPath: params?.projectPath,
    })
    ...
    Logger.info(
      "Index",
      `Project: ${context.projectName} (${context.language})`,
      { path: context.projectPath },
    )
    ...
    if (recent && !params.force) {
      ...
      if (recent.data.phase === "error" && ageMs < 10 * 60 * 1000) {
        const errMsg =
          recent.data.error || recent.data.message || "Pipeline failed"
      ...
      if (
        recent.data.phase === "awaiting_config_ui" &&
      ...
      if (
        [
          ...
          "auth_injected",
          "auth_refreshing",
          "auth_refresh_reused_preauth",
          "generating",
          "generation_partial",
          "generation_async_enqueued",
          "generation_async_progress",
          "running",
          "running_tests",
          "secondary_services_started",
          "tests_complete",
          "reporting",
          "uploading_artifacts",
          "artifacts_uploaded",
        ].includes(recent.data.phase) &&
        ageMs < 30 * 60 * 1000
      ) {
        return {
          content: [
    ...
    try {
      const {
        ...
        describePreflight,
      } = require("./port-preflight")
      const preflightDashboardUrl =
        process.env.HEALIX_DASHBOARD_URL || "http://localhost:3000"
      ...
      if (preflight.conflict && preflight.newPort) {
        Logger.warn(
          "Index",
          "Port pre-flight bumped target dev-server port before config UI",
          {
        ...
        baseConfig.baseURL =
          preflight.newBaseUrl || `http://localhost:${preflight.newPort}`
        this.writeRunStatus(statusFile, {
          runId,
        ...
        Logger.warn(
          "Index",
          "Port pre-flight detected a conflict but found no free port",
          {
            originalPort: preflight.originalPort,
            detectedAs: preflight.detectedAs,
          },
      ...
      Logger.warn("Index", "Port pre-flight failed (non-fatal)", {
        error: err?.message,
      })
    }
...

    {
      ...
      try {
        ...
        process.stderr.write(
          `[HEALIX] Config form: ${configUrl} — open and submit to start testing.\n`,
        )
      } catch (error) {
        this.writeRunStatus(statusFile, {
          runId,
      ...
      this.continuePipelineAfterConfig({
        ...
        .catch((err) => {
          ...
          Logger.error("Index", `Pipeline failure for run ${runId}`, {
            error: err?.message,
            code: err?.code,
          })
          try {
          ...
          process.stderr.write(
            `[HEALIX] Pipeline failed for run ${runId}: ${err?.message || err}\n`,
          )
        })
  ...
  async handleCheckRunStatus(params) {
            ...
            text: JSON.stringify(
              {
                success: false,
                error:
                  "runId is required (a string returned by healix_test_my_app).",
              },
              null,
              2,
            ),
            ...
            text: JSON.stringify(
              {
                ...
                agentInstructions:
                  "Status file was mid-write. Wait ~5 seconds and call healix_check_run_status again.",
              },
              null,
              2,
            ),
    ...
    const base = {
      ...
      dashboardUrl:
        status.dashboardUrl || process.env.HEALIX_DASHBOARD_URL || null,
    ...
    if (report) {
      try {
        const {
          buildAgentResponse,
        } = require("./failure-triage/agent-response")
        ...
        Logger.warn("Index", "buildAgentResponse failed in check_run_status", {
          error: err.message,
        })
      }
    }
    ...
    try {
      ...
      } = require("./failure-triage/error-remediations")
      ...
      Logger.warn("Index", "buildRemediationBlock failed", {
        error: err.message,
      })
    }
  ...
  async handleAnalyzeFailures(params) {
    Logger.mcp("Index", "handleAnalyzeFailures called", {
      projectPath: params?.projectPath,
    })

    const projectPath = params.projectPath || process.cwd()
    const testResultsPath =
      params.testResultsPath || `${projectPath}/test-results.json`
  ...
  loadLatestHealixReport(projectPath) {
    try {
      ...
      return JSON.parse(
        fs_.readFileSync(path_.join(reportsDir, files[0]), "utf-8"),
      )
    } catch {
      return null
    }

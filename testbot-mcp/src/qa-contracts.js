...
// ── C01: Persistent Tier-0 directory ────────────────────────────────────────
// Tier-0 specs live here and are NEVER deleted between runs.
...
// Surface hash file — tracks whether the app changed since the last run.
// If the hash matches, we skip regeneration entirely (diff-driven regen).
function surfaceHashPath(projectPath) {
  return path.join(projectPath, '.healix', 'tier0-surface-hash.json');
}

function computeSurfaceHash(spec) {
  // Hash the spec content so we can detect when contracts actually change.
  return crypto.createHash('sha256').update(spec.content || '').digest('hex');
}

function readStoredHash(projectPath) {
  try {
    const raw = fs.readFileSync(surfaceHashPath(projectPath), 'utf-8');
...
function writeStoredHash(projectPath, hash) {
  const dir = path.join(projectPath, '.healix');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(surfaceHashPath(projectPath), JSON.stringify({ hash, updatedAt: new Date().toISOString() }), 'utf-8');
}
...
function ensureQaContractSpec({ projectPath, context = {}, roles = [], testType = 'both' } = {}) {
  ...
  // ── Fix 2: Finding-ID-keyed filename ──────────────────────────────────────
  // Name the file by its content type so subsequent runs can identify it and
  // skip writing if nothing changed. Format: qac-<type>-<short-hash>.spec.ts
  const shortHash = crypto.createHash('sha256').update(spec.content).digest('hex').slice(0, 8);
  const idKeyedFilename = `qac-contracts-${shortHash}.spec.ts`;

  // ── Fix 1 + Fix 3: Write to persistent folder, never to ephemeral ─────────
  const persistDir = persistentTestsDir(projectPath);
  fs.mkdirSync(persistDir, { recursive: true });
  const targetPath = path.join(persistDir, idKeyedFilename);

  // ── Fix 4: Diff-driven regen ───────────────────────────────────────────────
  // Compute a hash of the new spec content. If it matches what we wrote last
  // time, the app surface hasn't changed — skip the write and return the
  // existing file so downstream callers still get a valid path.
  const newHash = computeSurfaceHash(spec);
  const oldHash = readStoredHash(projectPath);

  if (oldHash === newHash && fs.existsSync(targetPath)) {
    // Surface unchanged — reuse existing Tier-0 specs, no write needed.
    result.written = false;
    result.reused  = true;
    result.filename = idKeyedFilename;
    result.path = targetPath;
    result.generatedTests = (spec.content.match(/\btest\s*\(/g) || []).length;
    return result;
  }

  // Surface changed (or first run) — write fresh spec and update stored hash.
  fs.writeFileSync(targetPath, spec.content, 'utf-8');
  writeStoredHash(projectPath, newHash);

...
function ensureHealixValidationConfig({ projectPath, targetFilename = null } = {}) {
  ...
  // Normalise to forward slashes so the path is valid on Windows inside a
  // JS string literal and matches the cross-platform regex in tests.
  const generatedDirPosix = generatedDir.replace(/\\/g, '/');
...
module.exports = {
  testDir: ${JSON.stringify(generatedDirPosix)},
  testMatch: ${testMatch},
  timeout: 60000,
  fullyParallel: false,
  forbidOnly: false,
...
async function runPipeline(config, runId) {
  ...
  try {
    ...
    // -------------------------------------------------------
    // 0.5. Playwright pre-flight (before AI generation)
    // -------------------------------------------------------
    // Check that Playwright can actually run BEFORE spending the AI token
    // budget on test generation.  If it clearly cannot be resolved or
    // installed we throw here with an actionable message.
    {
      const _playwrightPI = new PlaywrightIntegration(config);
      const pwCheck = _playwrightPI.checkPlaywrightAvailable();
      if (!pwCheck.ok) {
        const err = new Error(
          `Playwright pre-flight failed: ${pwCheck.reason}\n\n` +
          'Run `npm install -D @playwright/test` in your project root, then\n' +
          '`npx playwright install chromium` to install the browser.',
        );
        err.code = 'PLAYWRIGHT_PREFLIGHT_FAILED';
        updateStatus(statusDir, 'playwright_preflight_failed', {
          runId,
          message: err.message,
          project: config.projectName,
          aiOnlyEnforced,
